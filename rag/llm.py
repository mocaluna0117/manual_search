...
ADMIN_TOOLS = [
    ...
    },
    {
        "toolSpec": {
            "name": "rename_folder",
            "description": (
                "既にあるフォルダ(カテゴリ)の名前を変える、または見せる範囲を変える。"
                "「〇〇フォルダの名前を△△に変えて」「フォルダ名を△△にして」の"
                "ような依頼に使う。新しく作り直すのではなく、今ある箱の名前を"
                "書き換えるので、中のマニュアルはそのまま残る"
            ),
            "inputSchema": {
                "json": {
                    "type": "object",
...
- 「フォルダ名を△△に変えて」のような名前の変更は、必ずrename_folderを使う。create_folderで作り直してはいけない(同じ中身の箱が2つでき、元の箱が空のまま残る)。直前の会話で作ったフォルダに対する変更依頼も同じ
